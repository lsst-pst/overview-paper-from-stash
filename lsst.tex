...
progresses, starting at
 $\sim$200 TFLOPS and peaking at $\sim$1 PFLOPS by the end of the survey. 
Processing such a large volume of data, converting the raw images into a faithful
...
Different aspects of data processing will occur at three facilities: the
Summit Facility at Cerro Pachon (initial cross-talk correction), the
Base Facility in La Serena (retransmition hub, also the data access
center for Chilean community), and the central Archive Facility at
NCSA, in Champaigne, IL which will also serve as a data access center
for the US community.

The data will be transported over existing high-speed optical fiber 
...
In addition to providing the hardware capabilities, LSST Data Management will
enable LSST science by creating a well documented, state-of-the-art, 
high-performance, scalable, multi-camera, open source, O/IR survey data 
processing and analysis system.

Algorithm development for LSST builds on the experience base and expertise
already gained in large astronomical surveys (e.g., SDSS, Pan-STARRS, DES, 
...
extraction of scientifically useful characteristics of those objects. Unfortunately, most 
of these codes are poorly suited for running at the scale of LSST, are highly optimized 
for the specific survey they were built for, or suffer from general lack of documentation 
and software engineering that makes them infeasible to extend and maintain over the
lifetime of LSST.

Therefore, while firmly footed in this prior history, the LSST data
management software has largely been written anew, in Python and (where necessary
for performance reasons) C++. Furthermore, all LSST codes have been
designed to follow best practices of software engineering (modularity, 
unit testing, etc.).

The LSST data management system has been prototyped over eight years, including undergoing eight data challenges processing data sets with varying degrees of difficulty. It currently ($\sim$ late 2014) consists of implementations of single frame processing pipelines (the 


%The principal functions of the DM system are to: %\begin{itemize}%%\item Process, in real time, the incoming stream of images generated by the camera system during observing by archiving raw images, generating alerts to sources whose properties have changed on the sky, and updating the relevant catalogs (``Level 1'' data products).%%\item Periodically process the accumulated survey data to provide a uniform photometric and astrometric calibration, measure the properties of fainter objects, and classify objects based on their time-dependent behavior. The results of such a processing run form a {\em data release} (DR), which is a static, self-consistent data set for use in performing scientific analysis of LSST data and publication of the results (``Level 2'' data products). All data releases are archived for the entire operational life of the LSST archive.%%\item Facilitate the creation of added-value (``Level 3'') data products, by providing suitable software, APIs, and computing infrastructure at the LSST data center.%%\item Make all LSST data available through an interface that utilizes, to the maximum possible extent, community-based standards. Provide enough processing, storage, and network bandwidth to enable user analysis of the data without petabyte-scale data transfers. %%\end{itemize}%%
%The rapid cadence of the LSST observing program will produce an enormous
%volume of data ($\sim$15 TB of raw imaging data per night). Over the ten years of
%operations and 11 data releases, this results in a cumulative {\em processed} data
%size approaching 0.5 EB for imaging, and over 50 PB for the catalog databases 
...
%The LSST data management system has been prototyped over eight years, including undergoing eight data challenges processing data sets with varying degrees of difficulty. It currently ($\sim$ late 2014) consists of implementations of single frame processing pipelines (the 
%
%The most recent data challenge
%
%Since 2006, LSST Data Management (data challenges...)
%%
%\begin{figure*}
%\hskip 0.5in
